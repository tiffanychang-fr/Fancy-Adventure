class LevelFour {
  ...
  play() {
    this.background.drawBackground("level 4");
  ...
  TileTypeCheck(player) {
    ...
    let tileType = this.map.levelFourTiles[this.positionX][this.positionY];
    ...
    if (tileType == 0) {
      ...
      if (
        ...
        if (
          ...
          this.map.levelFourTiles[this.positionX - 1][this.positionY] == 1
        ) {
  ...
  isCollidingTreasure(player, treasures) {
    treasures.coin_levelFour.forEach((coin, index) => {
      if (
        ...
        treasures.coin_levelFour.splice(index, 1);
        this.amountOfCoins++;
        COIN.innerText = this.amountOfCoins;
  ...
  isCollidingFruit(player, fruits) {
    fruits.fruitArray_levelFour.forEach((fruit, index) => {
      if (
  ...
  isCollidingEnemy(player, enemies) {
    enemies.enemyArray_levelFour.forEach((enemy, index) => {
      if (
